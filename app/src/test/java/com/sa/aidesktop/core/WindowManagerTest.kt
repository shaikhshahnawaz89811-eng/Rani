...
class WindowManagerTest {
    @Test fun openFocusMinimizeRestoreClose() {
    ...
    @Test fun maximizePreservesBoundsAndRestoreReturnsToThem() {
        val wm = DesktopWindowManager()
    ...
    @Test fun resizeNeverGoesBelowMinimum() {
        val wm = DesktopWindowManager()
...
@Test fun openNewCreatesIndependentBrowserWindows() {
    ...
    assert(first != second)
    assert(manager.windows.count { it.type == WindowType.BROWSER } == 2)
}


@Test fun resizeSupportsAllEdgesAndClampsAfterRotation() {
    val wm = DesktopWindowManager()
...
fun resizeIsClampedToWorkspace() {
    ...
    manager.initializeBounds(id, WindowBounds(10f, 10f, 300f, 200f))
    manager.resizeWithinWorkspace(id, ResizeEdge.RIGHT, 1000f, 0f, 500f, 400f)
    val w = manager.windows.single()
    assert(w.x >= 4f)
    assert(w.x + w.width <= 500f)
}
